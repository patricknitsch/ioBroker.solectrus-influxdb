...
- **InfluxDB Sensors grid**: Shows all enabled sensors as compact cards in a responsive grid. Each card shows:
  ...
  - **Timestamp row** (shown when alive timeout is configured): left-aligned **timestamp** (date and time when the sensor last received a new value); right-aligned **next expected update** as a badge (time only, auto-computed as timestamp + timeout interval — no manual input needed). When the current value is 0 the 60-minute fallback interval is used for the next-update calculation. The row is shown in **orange** when the alive timeout has been exceeded.
