...
- The tab is split into two sections, each with its own heading: **Sensors** (blue diamond icon, sensors actually written to InfluxDB) and **Internal Sensors** (🔒, only shown when at least one exists).
- **Sensors grid**: Shows all enabled sensors that are actually written to InfluxDB, as compact cards in a responsive grid. Each card shows:
