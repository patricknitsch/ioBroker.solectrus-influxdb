...
- Der Bereich trägt die allgemeine Überschrift **Sensoren** ohne weitere Unterüberschriften; die beiden Raster darunter werden allein durch ihre Kartenfarbe unterschieden (siehe Farblegende).
...
- **Interne Sensoren Raster** (nur sichtbar, wenn mindestens ein Sensor mit aktivierter **Internal**-Option existiert): Eigenes Raster unterhalb des InfluxDB-Sensoren-Rasters (ohne eigene Überschrift), in gleicher Kartendarstellung, aber mit **gelbem linken Rahmen** und **gelbem aktuellem Wert**. Da diese Sensoren nicht nach InfluxDB geschrieben werden, sind sie hier klar von den regulären (grünen) Sensoren getrennt.
