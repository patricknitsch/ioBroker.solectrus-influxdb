...
describe('backupManager', () => {
	...
	it('a dirOverride wins over the persisted config (unsaved dialog edits still work)', async () => {
		const staleBadDir = path.join(adapterDir, 'stale-bad-path.txt');
		await fsp.writeFile(staleBadDir, 'not a directory'); // simulates an old, now-broken saved path
		const overrideDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'solectrus-backup-override-'));

		const adapter = makeAdapter(adapterDir, { config: { backupDir: staleBadDir } });

		expect(backupManager.getBackupDir(adapter, overrideDir)).to.equal(overrideDir);
		const fileName = await backupManager.createBackup(adapter, overrideDir);
		expect(fs.existsSync(path.join(overrideDir, fileName))).to.equal(true);

		const backups = await backupManager.listBackups(adapter, overrideDir);
		expect(backups.map(b => b.fileName)).to.include(fileName);
