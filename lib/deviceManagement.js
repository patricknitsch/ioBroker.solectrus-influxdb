...
class SolectrusDeviceManagement extends DeviceManagement {
	...
	constructor(adapter) {
		super(adapter, true); // enables communicationStateId = 'info.deviceManager'
	...
	async _buildSensorDevice(sensor, state) {
		...
		customItems.timestamp = {
			...
			data:       tsDisplay,
			styleLabel: { fontWeight: 'bold' },
			styleValue: aliveMs ? { color: aliveColor } : {},
		};

		customItems.lastValue = {
			type:       'staticInfo',
			label:      { en: 'Last Value:', de: 'Letzter Wert:' },
			data:       lastValDisplay,
			styleLabel: { fontWeight: 'bold' },
			styleValue: {},
		};
			...
			return {
				...
				schema: {
					...
					items: {
						...
						sourceState: { type: 'text', label: { en: 'Source State', de: 'Quell-State' },   disabled: true, minRows: 2, maxRows: 4 },
			...
			return {
				...
				schema: {
					...
					items: {
						...
						formula:   { type: 'text', label: { en: 'Formula / Expression', de: 'Formel / Ausdruck' }, disabled: true, minRows: 2, maxRows: 6 },
						stateId:   { type: 'text', label: 'State ID',                                              disabled: true, minRows: 2, maxRows: 3 },
						timestamp: { type: 'text', label: { en: 'Last Update', de: 'Letzter Update' },             disabled: true },
					},
				},
			};
