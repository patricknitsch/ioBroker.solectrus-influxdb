...
class SolectrusDeviceManagement extends DeviceManagement {
	...
	async _buildSensorDevice(sensor, state) {
		...
		/* Value color: red > orange > yellow > green */
		const valueColor = isMaxExc ? COLOR_RED : isAliveStale ? COLOR_ORANGE : isInternal ? COLOR_YELLOW : COLOR_GREEN;
		...
		/* badge style helper */
		const badge = (color) => ({
			border:       `1px solid ${color || 'rgba(255,255,255,0.45)'}`,
			borderRadius: '4px',
			padding:      '1px 8px',
			fontSize:     '0.82em',
			color:        color || 'inherit',
		...
		/* Row 1: current value (label=left big) + max value badge (data=right) */
		const customItems = {
			value: {
				type:       'staticInfo',
				label:      valDisplay,
				data:       maxVal !== null ? `${maxVal}${unit ? ' ' + unit : ''}` : '',
				styleLabel: { color: valueColor, fontWeight: 'bold', fontSize: '1.3em' },
				styleValue: maxVal !== null ? badge(isMaxExc ? COLOR_RED : null) : {},
			},
			/* Row 2: empty left + sensor type badge (right) */
			typeTag: {
				type:       'staticInfo',
				label:      '',
				data:       sensor.type || 'float',
				styleValue: badge(null),
			},
		};

		/* Row 3: timestamp (label=left) + timeout badge (data=right) */
		if (lastTs) {
			customItems.timestamp = {
				type:       'staticInfo',
				label:      `Zeitstempel: ${tsDisplay}`,
				data:       nextUpdate || '',
				styleLabel: { fontSize: '0.82em', color: isAliveStale ? COLOR_ORANGE : 'inherit' },
				styleValue: nextUpdate ? badge(isAliveStale ? COLOR_ORANGE : null) : {},
			};
		}
				...
				schema: {
					...
					items: {
						value: {
							...
							label:      valDisplay,
							data:       '',
							styleLabel: { color: COLOR_GREEN, fontWeight: 'bold', fontSize: '1.3em' },
						},
						timestamp: {
							type:       'staticInfo',
							label:      `Zeitstempel: ${tsDisplay}`,
							data:       '',
							styleLabel: { fontSize: '0.82em' },
						},
					},
				},
			},
