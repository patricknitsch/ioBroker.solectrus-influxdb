...
const ADAPTER_ICON  = '/adapter/solectrus-influxdb/solectrus-influxdb.png';
...
class SolectrusDeviceManagement extends DeviceManagement {
	...
	async _buildSensorDevice(sensor, state) {
		...
		/* Zeitstempel = last received time; Letzter Wert = last received + timeout (= deadline) */
		const tsDisplay  = lastTs ? new Date(lastTs).toLocaleTimeString() : '–';
		const nextUpdate = (aliveMs && lastTs) ? new Date(lastTs + effectAliveMs).toLocaleTimeString() : null;
	...
	async _initCommState() {
		...
		/* Pre-populate lookup maps so notifyStateChange works before loadDevices is first called */
		this._populateMaps();
	}
