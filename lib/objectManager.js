...
async function migrateLegacyForecastConfig(adapter) {
	...
	if (enabled.length > 0) {
		...
		if (uniqueSources.length === 1) {
			// All entries share one source → one auto-detect sensor covers all fields
			const target = sensors.find(s => s && s.SensorName === 'INVERTER_POWER_FORECAST' && s.type === 'json');
			if (target && !target.sourceState) {
				target.sourceState = uniqueSources[0];
				target.enabled = true;
				adapter.log.info(
					`Migration: set sourceState "${uniqueSources[0]}" on INVERTER_POWER_FORECAST (auto-detect writes y, clearsky, temp)`,
				);
			} else if (!target) {
