...
async function ensureItemTitlesInInstanceConfig(adapter) {
	try {
		...
		const items = Array.isArray(obj.native.dsItems) ? obj.native.dsItems : [];
		const itemsEditor = Array.isArray(obj.native.dsItemsEditor) ? obj.native.dsItemsEditor : [];
		...
		// If Admin stored items under dsItemsEditor, migrate them back into dsItems
		// so runtime + fallback table see the same config.
		if (items.length === 0 && itemsEditor.length > 0) {
			obj.native.dsItems = itemsEditor;
			changed = true;
		}
