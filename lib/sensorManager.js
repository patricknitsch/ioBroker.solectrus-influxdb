...
async function prepareJsonSensors(adapter) {
	...
	for (const sensor of adapter.config.sensors) {
		...
		if (sensor.jsonPreset === 'auto' && (sensor.measurement || sensor.field)) {
			adapter.log.warn(
				`JSON sensor "${sensor.SensorName}": auto preset ignores measurement/field overrides – remove them from the config`,
			);
		}
