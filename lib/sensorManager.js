...
const PRESET_VALUES = Object.values(JSON_PRESETS);
...
/**
 * Creates or updates ioBroker state objects for all enabled sensors, reads initial values
 * from foreign states, populates the cache, and subscribes to source state changes.
 *
...
/**
 * Returns the resolved JSON sensor configuration for a given sensor definition.
 *
 * @param {object} sensor - Sensor configuration object.
 * @returns {object} Resolved JSON sensor config with tsField, valField, measurement, field, influxType and/or autoDetect.
 */
...
/**
 * Filters a raw JSON array value to only the timestamp and value fields relevant to the sensor.
 *
 * @param {object} adapter - The ioBroker adapter instance (used for logging).
 * @param {string|Array} jsonVal - Raw JSON value to filter.
 * @param {string} tsField - Name of the timestamp field.
 * @param {string} valField - Name of the value field.
 * @returns {string|null} JSON string of filtered entries or null on parse failure.
 */
...
/**
 * Filters a raw JSON array to entries that contain the timestamp field and at least one
 * known value field from any configured JSON preset (auto-detection mode).
 *
 * @param {object} adapter - The ioBroker adapter instance (used for logging).
 * @param {string|Array} jsonVal - Raw JSON value to filter.
 * @param {string} tsField - Name of the timestamp field.
...
function extractJsonSensorValuesAuto(adapter, jsonVal, tsField) {
	...
	const knownFields = PRESET_VALUES.map(p => p.valField);
	const filtered = [];
	for (const entry of data) {
...
/**
 * Builds the JSON source map and subscribes to foreign states for all enabled JSON-type sensors.
 *
 * @param {object} adapter - The ioBroker adapter instance.
 */
...
/**
 * Processes a JSON state change for all JSON sensors mapped to the given source state,
 * buffers the resulting InfluxDB points, and triggers a flush.
 *
 * @param {object} adapter - The ioBroker adapter instance.
 * @param {string} sourceState - The foreign state ID whose value changed.
 * @param {string|Array} jsonVal - The new JSON value from the source state.
 */
function processJsonSensorData(adapter, sourceState, jsonVal) {
	...
	// Pre-compute which preset fields appear in any entry (used for missing-field warnings below)
	const hasAutoMapping = mappings.some(m => !m.internal && m.autoDetect);
	const foundPresetFields = new Set();
	if (hasAutoMapping) {
		for (const entry of data) {
			if (!entry || typeof entry !== 'object') {
				continue;
			...
			for (const preset of PRESET_VALUES) {
				if (entry[preset.valField] != null) {
					foundPresetFields.add(preset.valField);
				}
			}
		}
	}
	...
	// Warn once per session when an expected preset field is completely absent from the JSON data
	if (hasAutoMapping) {
		if (!adapter.warnedMissingJsonFields) {
			adapter.warnedMissingJsonFields = new Set();
		}
		for (const preset of PRESET_VALUES) {
			const warnKey = `${sourceState}|${preset.valField}`;
			if (!foundPresetFields.has(preset.valField)) {
				if (!adapter.warnedMissingJsonFields.has(warnKey)) {
					adapter.warnedMissingJsonFields.add(warnKey);
					const msg = getNotificationMessage(adapter, 'jsonFieldMissing', {
						field: preset.valField,
						source: sourceState,
					});
					adapter.log.warn(msg);
					if (adapter.config.notifyOnJsonFieldMissing) {
						sendNotification(adapter, msg).catch(() => {});
					}
				}
			} else {
				// Field reappeared (e.g. backend switch) – reset so user is informed if it disappears again
				adapter.warnedMissingJsonFields.delete(warnKey);
			}
		}
	}
...
/**
 * Disables the sensor that caused an InfluxDB field-type-conflict error and logs the event.
 *
 * @param {object} adapter - The ioBroker adapter instance.
 * @param {Error} err - The InfluxDB write error containing the field-type-conflict message.
 */
async function disableSensorByFieldTypeConflict(adapter, err) {
	...
	// Persist disabled state to adapter config so it survives restarts
	try {
		...
		if (obj && obj.native && Array.isArray(obj.native.sensors)) {
			const persistedSensor = obj.native.sensors.find(
				s => s && s.measurement === measurement && s.field === field,
			);
			if (persistedSensor) {
				persistedSensor.enabled = false;
				await adapter.setForeignObjectAsync(objId, obj);
				adapter.log.info(`Sensor "${sensor.SensorName}" disabled state persisted to adapter config`);
			}
		}
	} catch (persistErr) {
		adapter.log.warn(`Failed to persist sensor disabled state: ${persistErr.message}`);
	}
}
