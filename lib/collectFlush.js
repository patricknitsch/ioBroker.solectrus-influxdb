				...
				if (now - lastWarnTs >= throttleMs) {
					...
					if (currentValue === 0 || currentValue === '0') {
						...
						if (adapter.config.notifyOnSensorTimeout) {
							const notifyRepeatMs = (adapter.config.notifyAliveTimeoutRepeatMinutes || 60) * 60_000;
...
function handleFlushFailure(adapter) {
	...
	if (adapter.config.notifyOnConnectionFail) {
		const repeatMs = (adapter.config.notifyConnectionFailRepeatMinutes || 60) * 60_000;
		const now = Date.now();
