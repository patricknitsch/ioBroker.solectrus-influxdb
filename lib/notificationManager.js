	...
	if (adapter.config.notifyUseTelegram) {
		try {
			if (!await isAdapterRunning(adapter, 'telegram')) {
				adapter.log.warn(
					'[sendNotification] No running Telegram instance found. Message could not be sent.',
				);
			} else {
				const payload = { text };
	...
	if (adapter.config.notifyUsePushover) {
		try {
			if (!await isAdapterRunning(adapter, 'pushover')) {
				adapter.log.warn(
					'[sendNotification] No running Pushover instance found. Message could not be sent.',
				);
			} else {
				const payload = {
					message: text,
	...
	if (adapter.config.notifyUseWhatsapp) {
		try {
			if (!await isAdapterRunning(adapter, 'whatsapp-cmb')) {
				adapter.log.warn(
					'[sendNotification] No running WhatsApp instance found. Message could not be sent.',
				);
			} else {
				const payload = { text };
	...
	if (adapter.config.notifyUseEmail) {
		try {
			if (!await isAdapterRunning(adapter, 'email')) {
				adapter.log.warn(
					'[sendNotification] No running Email instance found. Message could not be sent.',
				);
			} else {
				const emailPayload = {
					text,
	...
	if (adapter.config.notifyUseSignal) {
		try {
			if (!await isAdapterRunning(adapter, 'signal-cmb')) {
				adapter.log.warn(
					'[sendNotification] No running Signal instance found. Message could not be sent.',
				);
			} else {
				const payload = { text };
	...
	if (adapter.config.notifyUseMatrix) {
		try {
			if (!await isAdapterRunning(adapter, 'matrix-org')) {
				adapter.log.warn(
					'[sendNotification] No running Matrix instance found. Message could not be sent.',
				);
			} else {
	...
	if (adapter.config.notifyUseSynochat) {
		try {
			if (!adapter.config.notifyChannelSynochat) {
				adapter.log.warn(
					'[sendNotification] Synology Chat channel is not set. Message could not be sent.',
				);
			} else {
