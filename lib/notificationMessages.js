...
const messages = {
	en: {
		...
		jsonFieldMissing:
			'JSON sensor (auto): field "{field}" not found in data from "{source}" – sensors depending on this field will not write to InfluxDB',
	},
	de: {
		...
		jsonFieldMissing:
			'JSON-Sensor (Auto): Feld „{field}" in Daten von „{source}" nicht gefunden – Sensoren für dieses Feld schreiben keine Daten in InfluxDB',
	},
	fr: {
		...
		jsonFieldMissing:
			"Capteur JSON (auto) : champ « {field} » non trouvé dans les données de « {source} » – les capteurs dépendant de ce champ n'écriront rien dans InfluxDB",
	},
	pl: {
		...
		jsonFieldMissing:
			'Czujnik JSON (auto): pole „{field}" nie znalezione w danych z „{source}" – sensory zależne od tego pola nie zapisują danych do InfluxDB',
	},
	ru: {
		...
		jsonFieldMissing:
			'JSON-сенсор (авто): поле „{field}" не найдено в данных из „{source}" – сенсоры, зависящие от этого поля, не записывают данные в InfluxDB',
	},
	it: {
		...
		jsonFieldMissing:
			'Sensore JSON (auto): campo „{field}" non trovato nei dati da „{source}" – i sensori che dipendono da questo campo non scrivono dati in InfluxDB',
	},
	pt: {
		...
		jsonFieldMissing:
			'Sensor JSON (auto): campo „{field}" não encontrado nos dados de „{source}" – sensores dependentes deste campo não escreverão dados no InfluxDB',
	},
	uk: {
		...
		jsonFieldMissing:
			'JSON-сенсор (авто): поле „{field}" не знайдено в даних з „{source}" – сенсори, що залежать від цього поля, не записують дані в InfluxDB',
	},
	nl: {
		...
		jsonFieldMissing:
			'JSON-sensor (auto): veld „{field}" niet gevonden in gegevens van „{source}" – sensoren die afhankelijk zijn van dit veld schrijven geen data naar InfluxDB',
	},
	es: {
		...
		jsonFieldMissing:
			'Sensor JSON (auto): campo „{field}" no encontrado en datos de „{source}" – los sensores dependientes de este campo no escribirán datos en InfluxDB',
	},
	'zh-cn': {
		...
		jsonFieldMissing:
			'JSON传感器（自动）：在来自"{source}"的数据中未找到字段"{field}" – 依赖该字段的传感器将不会向InfluxDB写入数据',
	},
};

/** Emoji prefix to prepend to each message type. */
const prefixes = {
	influxConnectionFailed: '❗ ',
	influxConnectionRestored: '✅ ',
	sensorTimeout: '⚠️ ',
	maxValueExceededNoLastValid: '⚠️ ',
	maxValueExceeded: '⚠️ ',
	jsonFieldMissing: '⚠️ ',
};
...
function getNotificationMessage(adapter, key, params) {
	...
	const resolved = template.replace(/\{(\w+)\}/g, (_, k) => (params[k] !== undefined ? String(params[k]) : `{${k}}`));
