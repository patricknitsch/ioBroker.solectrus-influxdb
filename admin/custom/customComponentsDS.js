	...
	function createDataSolectrusItemsEditor(React, AdapterReact) {
		return function DataSolectrusItemsEditor(props) {
			...
			const getDomThemeType = () => {
				try {
					...
					if (root && root.classList) {
						if (root.classList.contains('mui-theme-dark') || root.classList.contains('iob-theme-dark'))
							return 'dark';
						if (root.classList.contains('mui-theme-light') || root.classList.contains('iob-theme-light'))
							return 'light';
					}
				} catch {
					// ignore
			...
			const paletteMatches = !!(
				themePalette &&
				(themePalette.mode === 'dark' || themePalette.mode === 'light') &&
				themePalette.mode === themeType
			);
			...
			const colors = Object.assign({}, fallbackColors, {
				panelBg:
					...
					fallbackColors.panelBg,
				panelBg2:
					(effectivePalette &&
						effectivePalette.background &&
						(effectivePalette.background.paper || effectivePalette.background.default)) ||
					fallbackColors.panelBg2,
				text:
					(effectivePalette && effectivePalette.text && effectivePalette.text.primary) || fallbackColors.text,
				textMuted:
					(effectivePalette && effectivePalette.text && effectivePalette.text.secondary) ||
				...
				hover:
					(effectivePalette && effectivePalette.action && effectivePalette.action.hover) ||
					fallbackColors.hover,
				active:
					(effectivePalette && effectivePalette.action && effectivePalette.action.selected) ||
			...
			const setByPath = (rootObj, path, value) => {
				...
				const clonedRoot = Array.isArray(rootObj) ? rootObj.slice() : Object.assign({}, rootObj || {});
			...
			const formulaLiveSignature = (() => {
				...
				const parts = ids.map(id => {
					const ts =
						formulaLiveTs && Object.prototype.hasOwnProperty.call(formulaLiveTs, id)
							? formulaLiveTs[id]
							: undefined;
					const val =
						formulaLiveValues && Object.prototype.hasOwnProperty.call(formulaLiveValues, id)
							? formulaLiveValues[id]
							: undefined;
			...
			const refreshFormulaLiveValues = async opts => {
				...
				const inputs = Array.isArray(selectedItem.inputs) ? selectedItem.inputs : [];
				const ids = inputs.map(inp => (inp && inp.sourceState ? String(inp.sourceState) : '')).filter(Boolean);
			...
			const insertIntoFormulaDraft = opts => {
				...
				const startWithin =
					opts && typeof opts.selectStartWithinText === 'number' ? opts.selectStartWithinText : text.length;
				const endWithin =
					opts && typeof opts.selectEndWithinText === 'number' ? opts.selectEndWithinText : text.length;
			...
			const setDraftInputField = (index, field, value) => {
				setSelectedDraft(prev => {
					...
					const cur =
						inputs[index] && typeof inputs[index] === 'object' ? Object.assign({}, inputs[index]) : {};
					cur[field] = value;
					inputs[index] = cur;
					next.inputs = inputs;
					return next;
						...
						if (selectContext.kind === 'formulaFn') {
							...
							if (fn === 's') {
								insertIntoFormulaDraft({
									text: `s("${selectedStr}")`,
									selectStartWithinText: `s("`.length + String(selectedStr).length + 2,
									selectEndWithinText: `s("`.length + String(selectedStr).length + 2,
								});
								return;
							}
							if (fn === 'v') {
								insertIntoFormulaDraft({
									text: `v("${selectedStr}")`,
									selectStartWithinText: `v("`.length + String(selectedStr).length + 2,
									selectEndWithinText: `v("`.length + String(selectedStr).length + 2,
								});
								return;
							}
							if (fn === 'jp') {
								...
								insertIntoFormulaDraft({
									text: txt,
									selectStartWithinText: start,
									selectEndWithinText: start + '$.value'.length,
								});
								return;
							}
						}
						...
						React.createElement(
							...
							React.createElement(
								...
								React.createElement(
									...
									t('Insert building blocks on the left. The editor uses current (unsaved) inputs.'),
								),
							),
							React.createElement(
								'button',
							...
							React.createElement(
								...
								vars.length
									? vars.map((v, idx) => {
											...
											const liveVal = liveId
												? computePreviewInputValue(selectedItem, v, rawLiveVal)
												: undefined;
									...
									React.createElement(
										'button',
										{
											...
											onClick: () =>
												insertIntoFormulaDraft({
													text: 'min(a, b)',
													selectStartWithinText: 4,
													selectEndWithinText: 5,
												}),
											title: t('fn.min'),
										},
										t('min'),
								...
								React.createElement(
									...
									React.createElement(
										'button',
										{
											type: 'button',
											style:
												DialogSelectID && socket && theme ? chipBtnStyle : chipBtnDisabledStyle,
											disabled: !(DialogSelectID && socket && theme),
							...
							React.createElement(
									...
									React.createElement(
										...
										React.createElement(
											'button',
											{
												...
												onClick: () =>
													refreshFormulaPreview({ reason: 'manual', showLoading: true }),
										...
										formulaPreview && formulaPreview.ok
											? React.createElement(
													'span',
													{
														style: previewOkPillStyle,
														title: stringifyCompact(formulaPreview.value, 200),
													},
											...
											: formulaPreview && !formulaPreview.ok
												? React.createElement(
														'span',
														{
															style: previewErrPillStyle,
															title: formulaPreview.error
																? String(formulaPreview.error)
																: '',
														},
														formulaPreview.error
															? stringifyCompact(formulaPreview.error)
															: t('n/a'),
													)
										...
										React.createElement(
											...
											tokenizeFormula(formulaDraft).map((token, i) => {
												...
												if (token.type === 'keyword')
													tokenColor = isDark ? '#c792ea' : '#9334e9';
												if (token.type === 'function')
													tokenColor = isDark ? '#82aaff' : '#2563eb';
												if (token.type === 'variable')
													tokenColor = isDark ? '#82e0aa' : '#16a34a';
												if (token.type === 'number')
													tokenColor = isDark ? '#f78c6c' : '#ea580c';
												if (token.type === 'string')
													tokenColor = isDark ? '#c3e88d' : '#65a30d';
												if (token.type === 'operator')
													tokenColor = isDark ? '#89ddff' : '#0891b2';
								...
								React.createElement(
									...
									React.createElement(
										...
										React.createElement(
											'button',
											{ type: 'button', style: btnStyle, onClick: close },
											t('Cancel'),
										),
										React.createElement(
											'button',
											{ type: 'button', style: btnStyle, onClick: apply },
											t('Apply'),
										),
									),
								),
												...
												React.createElement(
													'div',
													{
														style: dropdownItemStyle(
															(selectedItem.mode || 'formula') === 'formula',
														),
														onClick: () => {
								...
								(selectedItem.mode || 'formula') === 'source'
									...
									: selectedItem.mode === 'state-machine'
										? React.createElement(
												...
												(Array.isArray(editItem.inputs) ? editItem.inputs : []).map(
													(inp, idx) =>
														React.createElement(
															'div',
															{
															...
															React.createElement('input', {
																...
																placeholder: t('Key'),
																onChange: e =>
																	setDraftInputField(idx, 'key', e.target.value),
															...
															React.createElement('input', {
																...
																onBlur: e =>
																	updateInput(idx, 'sourceState', e.target.value),
															}),
															React.createElement('input', {
																style: inputStyle,
																type: 'text',
																...
																onChange: e =>
																	setDraftInputField(idx, 'jsonPath', e.target.value),
																onBlur: e =>
																	updateInput(idx, 'jsonPath', e.target.value),
																title: t('e.g. $.apower'),
															}),
															React.createElement(
																'div',
																...
																renderSelectButton(() =>
																	setSelectContext({ kind: 'input', index: idx }),
																),
															),
															React.createElement(
																'button',
												...
												React.createElement(
													...
													React.createElement(
														'button',
														{
															...
															style: btnStyle,
															onClick: () => {
																const rules = Array.isArray(editItem.rules)
																	? editItem.rules.slice()
																	: [];
													...
													React.createElement(
														...
														React.createElement(
															'button',
															{
																...
																onClick: () => {
																	const rules = Array.isArray(selectedItem.rules)
																		? [...selectedItem.rules]
																		: [];
																	const itemType = selectedItem.type || 'string';
																	if (itemType === 'boolean') {
																...
																title:
																	(selectedItem.type || 'string') === 'boolean'
																		? 'Battery OK check'
												...
												(Array.isArray(editItem.rules) ? editItem.rules : []).map(
													(rule, ruleIdx) =>
														React.createElement(
															'div',
															{
															...
															React.createElement('input', {
																...
																onBlur: () =>
																	updateSelected(
																		'rules',
																		Array.isArray(editItem.rules)
																			? editItem.rules
																			: [],
																	),
																placeholder: t('e.g. soc < 10 or true for default'),
																title: t(
																	'Formula syntax: soc < 10, battery > 80 && surplus > 0, true (for default/fallback)',
																),
															}),
															React.createElement(
																'label',
										...
										React.createElement(
											...
											React.createElement(
												'div',
												{
													...
													onClick: () =>
														setOpenDropdown(
															openDropdown === 'datatype' ? null : 'datatype',
														),
													onKeyDown: e => {
														if (e && (e.key === 'Enter' || e.key === ' ')) {
															e.preventDefault();
								...
								selectedItem.mode !== 'state-machine'
									? React.createElement(
											...
											React.createElement(
												...
												t(
													selectedItem && selectedItem.mode === 'formula'
