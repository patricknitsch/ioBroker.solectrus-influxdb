...
class SolectrusInfluxdb extends utils.Adapter {
	constructor(options) {
		...
		/* Device Manager (dm-utils) – registriert eigenen message-Listener */
	...
	async onReady() {
		...
		/* Set up DM communication state (and lookup maps) before prepareSensors subscribes */
		await this.deviceManagement._initCommState().catch(() => {});
	...
	onMessage(obj) {
		try {
			...
			/* dm:* wird von SolectrusDeviceManagement (dm-utils) verarbeitet */
