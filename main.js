...
class SolectrusInfluxdb extends utils.Adapter {
	...
	onMessage(obj) {
		try {
			...
			if (obj.command === 'deleteStates') {
				this._handleDeleteStates(obj).catch(e => {
					try {
						if (obj.callback) {
							this.sendTo(
								obj.from,
	...
	/* =====================================================
	 * MANAGEMENT COMMANDS (sendTo)
	 * ===================================================== */

	async _handleInitStates(obj) {
		try {
			this.log.info('[Management] initStates command received – reinitializing sensor states…');
			await this.retryOnConnectionError(() => this.prepareSensors(), 'initStates: prepareSensors');
			await this.retryOnConnectionError(
				() => this.prepareForecastSources(),
				'initStates: prepareForecastSources',
			);
			...
			if (this.config.enableDataSolectrus) {
				if (!this.dsProxy) {
					...
					await this.retryOnConnectionError(
						() => dsItemManager.prepareItems(this.dsProxy),
						'initStates: prepareItems',
					);
					this.log.info('[Management] Data-SOLECTRUS items reinitialized');
				}
			}

			this.log.info('[Management] initStates completed successfully');
			if (obj.callback) {
				this.sendTo(obj.from, obj.command, { ok: true }, obj.callback);
			}
		} catch (e) {
			this.log.error(`[Management] initStates failed: ${e.message || String(e)}`);
			if (obj.callback) {
				this.sendTo(obj.from, obj.command, { ok: false, error: e.message || String(e) }, obj.callback);
			}
		}
	...
	async _handleDeleteStates(obj) {
		try {
			this.log.info('[Management] deleteStates command received – deleting adapter states…');
			let deleted = 0;
			...
			// Delete DS states (entire ds.* subtree)
			if (this.config.enableDataSolectrus) {
				try {
					const dsObjects = await this.getForeignObjectsAsync(`${this.namespace}.ds.*`);
					if (dsObjects) {
						const ids = Object.keys(dsObjects).sort((a, b) => b.localeCompare(a)); // delete deepest first
						for (const id of ids) {
							try {
					...
					// Delete ds channel itself
					try {
						await this.delObjectAsync('ds');
						deleted++;
					} catch {
						// ignore
					}
			...
			this.log.info(`[Management] deleteStates completed – ${deleted} object(s) deleted`);
			if (obj.callback) {
				this.sendTo(obj.from, obj.command, { ok: true, deleted }, obj.callback);
			}
		} catch (e) {
			this.log.error(`[Management] deleteStates failed: ${e.message || String(e)}`);
			if (obj.callback) {
				this.sendTo(obj.from, obj.command, { ok: false, error: e.message || String(e) }, obj.callback);
			}
		}
